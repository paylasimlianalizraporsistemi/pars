...
class Validate extends Component {
  ...
  async validate(reportID) {
    /*
    var validate = await blockchain.validateReport(reportID)

    console.log(validate)
    */
    console.log('validateeeeeeeeee')
  }
  ...
  async getReports() {
    const reportCount = await this.state.contract.methods
      .getReportCount()
      .call()
    var gett = await this.state.contract.methods.getAllReports().call()
    console.log(gett)
    ...
    for (var i = 0; i < reportCount; i++) {
      if (gett[i][1] == '') {
        ...
        const validatediv = document.createElement('button')

        var reportID = gett[i][0]

        validatediv.onclick = function () {
          var validate = blockchain.validateReport(reportID)

          console.log(validate)
        }

        validatediv.classList.add('col')
            ...
            <div className="row files-title">
              <div className="col rounded p-3 mb-5 shadow bg-white">
                <div class="row ">
                  ...
                  <div class="col">
                    <div className="border-bottom">Validation</div>
                  </div>
                </div>
              </div>
            </div>
